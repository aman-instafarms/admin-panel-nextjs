...
export default async function Page({ searchParams, params }: ServerPageProps) {
  ...
  const query = db
    .select({ ...userFields, propertyId: managersOnProperties.propertyId })
    .from(users)
    .leftJoin(
      managersOnProperties,
      and(
        eq(users.id, managersOnProperties.managerId),
        eq(managersOnProperties.propertyId, idString),
      ),
    );
        ...
        <div className="space-between flex w-full flex-row items-center">
          <div className="flex w-full flex-col gap-2">
            <h5 className="text-xl font-bold tracking-tight text-gray-900 dark:text-white">
              Add Manager - {property.propertyCode || property.id}
            </h5>
            ...
            <Breadcrumb className="bg-white pb-3 dark:bg-gray-800">
              ...
              <BreadcrumbItem href={`/admin/properties/${idString}/managers`}>
                Managers
              </BreadcrumbItem>
              <BreadcrumbItem href="#">Add Manager</BreadcrumbItem>
            </Breadcrumb>
          </div>
